...
function AppLayout({ title, subtitle, actions, children }) {
  ...
  useEffect(() => {
    const handleLock = (event) => setTransactionLock(event.detail?.locked ? event.detail : null)
    window.addEventListener('vericargo:transaction-lock', handleLock)
    return () => window.removeEventListener('vericargo:transaction-lock', handleLock)
  }, [])
  ...
  return <div className="app-shell app-shell--hybrid">
    <header className="system-bar system-bar--dashboard-link" onClick={(event) => { if (!event.target.closest('button, a, [role="menu"]')) navigate('/dashboard') }}>
    ...
    {transactionLock && <div className="transaction-lock" role="alert" aria-live="assertive" aria-busy="true"><div className="transaction-lock__card"><span className="state-spinner state-spinner--large" aria-hidden="true" /><h2>Transaction in progress</h2><p>{transactionLock.message}</p><small>Do not close, refresh, or navigate away from this page.</small></div></div>}
  </div>
}
